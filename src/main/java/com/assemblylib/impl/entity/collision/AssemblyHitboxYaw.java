 ...
 * block grid — a port of sable's {@code SubLevelEntityCollision.getHitBoxYaw} plus the
 * {@code SableMathUtils.clampQuaternionToGrid(.., GridQuats.REAL)} it relies on.
 *
 * <p>The block grid is 90°-symmetric, so the box only needs aligning to the assembly's rotation
 * <em>modulo</em> the nearest 90° grid snap. Snapping the assembly orientation to the closest cube
 * rotation and taking the residual yaw gives exactly that: at 0/90/180/270° the box is axis-aligned;
 * in between it tilts to stay parallel to the (rotated) block faces, so the entity slides along a
 * spinning platform instead of catching on box corners.</p>
 */
