...
/**
 * Describes the current rotation of a assembly for the collider. Mirrors the
 * relevant parts of Create's AssemblyRotationState. {@link #asMatrix()}
 ...
 */
public class AssemblyRotationState {

	public static final AssemblyRotationState NONE = new AssemblyRotationState(null, 0);

	public float xRotation = 0;
	public float yRotation = 0;
	public float zRotation = 0;

	public AssemblyRotationState(Axis axis, float angle) {
		if (axis == Axis.X)
			xRotation = angle;
		else if (axis == Axis.Y)
			yRotation = angle;
	 ...
	 * box orientation so it shares the assembly's Y rotation (stays yaw-aligned with a
	 * turntable instead of keeping its world-aligned footprint). For X/Z-axis assemblys
