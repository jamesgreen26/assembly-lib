...
/**
 * GPU-instanced rendering of a Servo Motor's assembly while Flywheel is
 * active, modelled on Create's {@code AssemblyVisual}. The structure model
 * and the child block-entity visuals live on a {@link VisualEmbedding} that
 * carries the assembly transform, and lighting is provided by registering the
 * assembly's world light sections as a {@link ShaderLightVisual} (the block
...
public class ServoMotorVisual extends AbstractBlockEntityVisual<ServoMotorBlockEntity>
	...
	private void setupStructure() {
		builtAssembly = blockEntity.getAssembly();
		if (structure != null) {
			structure.delete();
			structure = null;
		}
		...
		Model model = new BlockModelBuilder(
			new AssemblyRenderWorld(blockEntity.getLevel(), builtAssembly),
	...
	private void setupChildren(float partialTick) {
		...
		AssemblyRenderState renderState = blockEntity.getRenderState();
	...
	private void beginFrame(DynamicVisual.Context ctx) {
		...
		if (blockEntity.getAssembly() != builtAssembly)
			setupStructure();
	...
	private void setEmbeddingTransform(float partialTick) {
		...
		BlockPos vp = blockEntity.getLevel() instanceof AssemblyRenderLevel
			? blockEntity.getBlockPos()
			: getVisualPosition();

		Matrix4f pose = new Matrix4f();
		pose.translate(vp.getX() + facing.getStepX(), vp.getY() + facing.getStepY(), vp.getZ() + facing.getStepZ());
		AssemblyTransform.pivotRotate(pose, blockEntity.getRotationAxis(), angle);
